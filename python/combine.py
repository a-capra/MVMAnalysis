...
def process_run(args, meta, sitename, objname, input_mvm, fullpath_rwa, fullpath_dta, columns_rwa, columns_dta):
  ...
  if args.ignore_sim :
    if args.plot :
      plot_mvm_only_canvases(dfhd, meta, sitename, objname, output_directory, start_times, colors, args.web)
  ...
  if args.plot :
    ...
    ####################################################
    plot_service_canvases (df, dfhd, meta, sitename, objname, args.output_directory, start_times, colors, args.web, respiration_rate, inspiration_duration)

    ####################################################
    '''formatted plots for ISO std / arXiv. Includes 3 view plot and 30 cycles view'''
    ####################################################
    plot_arXiv_canvases (df, dfhd, meta, sitename, objname, args.output_directory, start_times, colors, args.web)
    ...
    for i in range (len(meta)) :
      ...
      ####################################################
      plot_summary_canvases (df, dfhd, meta, sitename, objname, args.output_directory, start_times, colors, args.web, measured_peeps, measured_plateaus, real_plateaus, measured_peaks, measured_volumes, real_tidal_volumes)
      plot_overlay_canvases ( dftmp, dfhd, meta, sitename, objname, args.output_directory, start_times, colors, args.web, stats_total_vol, stats_total_flow, stats_airway_pressure )
...
if __name__ == '__main__':
  ...
  if args.input[0].split('.')[-1]== 'json' :
    ...
    for input in args.input :
      ...
      process_run(args, meta, sitename, objname=objname, input_mvm=fname, fullpath_rwa=fullpath_rwa, fullpath_dta=fullpath_dta, columns_rwa=columns_rwa, columns_dta=columns_dta)
